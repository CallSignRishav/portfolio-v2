...
- **Email server action** lives in `src/Hooks/email.ts` (not `src/app/api/`). It uses `"use server"` directly, called from a client component via `server@server` import convention.
...
- **Projects section is commented out** in `AllSections.tsx` (lines 180-207).
...
- **CSS variables** for theming in `globals.css` — dark mode via `.dark` class (`next-themes` installed but no provider visible in layout).
